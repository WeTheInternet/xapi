...
public class EqualsVisitor implements GenericVisitor<Boolean, Node> {
	...
	@Override public Boolean visit(final TemplateLiteralExpr n1, final Node arg) {
		final TemplateLiteralExpr n2 = (TemplateLiteralExpr) arg; // allow a template to match a string if their values equal.
	...
	@Override
	public Boolean visit(UiBodyExpr n, Node arg) {
		UiBodyExpr n2 = (UiBodyExpr) arg;
		if (!nodesEquals(n.getChildren(), n2.getChildren())) {
			return Boolean.FALSE;
		}
		return Boolean.TRUE;
	}
	...
	@Override
	public Boolean visit(UiAttrExpr n, Node arg) {
		UiAttrExpr n2 = (UiAttrExpr) arg;
		...
		if (n.isAttribute() != n2.isAttribute()) {
			return Boolean.FALSE;
		}
		return Boolean.TRUE;
	}
	...
	@Override
	public Boolean visit(UiContainerExpr n, Node arg) {
		UiContainerExpr n2 = (UiContainerExpr) arg;
		if (!nodeEquals(n.getNameExpr(), n2.getNameExpr())) {
			return Boolean.FALSE;
		}
		if (!nodesEquals(n.getAttributes(), n2.getAttributes())) {
			return Boolean.FALSE;
		}
		if (!nodeEquals(n.getBody(), n2.getBody())) {
			return Boolean.FALSE;
		}
		if (n.isInTemplate() != n2.isInTemplate()) {
			return Boolean.FALSE;
		}
		return Boolean.TRUE;
	}
	...
	@Override
	public Boolean visit(SysExpr sysExpr, Node arg) {
		final SysExpr yourExpr = ((SysExpr) arg);
		final Iterator<Node> yourNodes = yourExpr.nodes()
			.iterator();
		final Iterator<Node> myNodes = sysExpr.nodes()
			.iterator();

		while (myNodes.hasNext()) {
			if (!yourNodes.hasNext()) {
				return false;
			}
			final Node myNode = myNodes.next();
			final Node yourNode = yourNodes.next();
			Boolean equal = myNode.accept(this, yourNode);
			if (Boolean.FALSE.equals(equal)) {
				return false;
			}
		}
		return !myNodes.hasNext();
	}

	@Override
	public Boolean visit(DynamicDeclarationExpr n, Node arg) {
