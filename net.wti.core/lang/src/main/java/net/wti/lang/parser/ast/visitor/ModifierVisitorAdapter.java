...
public abstract class ModifierVisitorAdapter<A> implements GenericVisitor<Node, A> {
	...
	@Override public Node visit(final ClassOrInterfaceType n, final A arg) {
		...
		final TypeArguments typeArgs = n.getTypeArguments();
		if (typeArgs != null && !typeArgs.isUsingDiamondOperator()) {
			List<Type> newTypes = new ArrayList<>();
			final List<Type> args = typeArgs.getTypeArguments();
			for (int i = 0; i < args.size(); i++) {
				newTypes.add((Type) args.get(i).accept(this, arg));
			}
	...
	@Override public Node visit(final TemplateLiteralExpr n, final A arg) {
		return n;
	}
