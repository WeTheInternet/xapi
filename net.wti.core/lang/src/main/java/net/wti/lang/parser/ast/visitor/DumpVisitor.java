...
import static net.wti.lang.parser.PositionUtils.sortByBeginPosition;
...
//        }
    ...
    public void visit(final PackageDeclaration n, final Object arg) {
        ...
        if (printer instanceof ClassBuffer) {
            ((ClassBuffer)printer).setPackage(n.getPackageName());
        } else {
    ...
    public void visit(final ClassOrInterfaceDeclaration n, final Object arg) {
        ...
        if (shouldInitializeBuffer(n, printer)) {
            // The classbuffer represents the type we are printing
            ClassBuffer cb = (ClassBuffer) printer;
            cb.setDefinition(
                Modifier.toString(n.getModifiers())
            ...
            if (n.isInterface()) {
                if (!isNullOrEmpty(n.getExtends())) {
                    n.getExtends().forEach(type->
                        cb.addInterface(resolveType(type))
                    );
                }
                if (!isNullOrEmpty(n.getImplements())) {
                    n.getImplements().forEach(type->
                        cb.addInterface(resolveType(type))
                    );
                }
            } else {
                if (!isNullOrEmpty(n.getExtends())) {
                    assert n.getExtends().size() == 1;
                    cb.setSuperClass(resolveType(n.getExtends().get(0)));
                }
                if (!isNullOrEmpty(n.getImplements())) {
                    n.getImplements().forEach(type->
                        cb.addInterface(resolveType(type))
    ...
    private boolean shouldInitializeBuffer(ClassOrInterfaceDeclaration n, Printable printer) {
        if (printer instanceof ClassBuffer) {
            ClassBuffer cb = (ClassBuffer) printer;
            String name = n.getName();
            return cb.getSimpleName() == null || name.equals(cb.getSimpleName());
        }
        return false;
    }
    ...
    public void visit(final VariableDeclaratorId n, final Object arg) {
        printJavaComment(n.getComment(), arg);
        printer.print(resolveName(new NameExpr(n.getName())));
        for (int i = 0; i < n.getArrayCount(); i++) {
            printer.print("[]");
        }
    }
...
//        printer.outdent();
    ...
    @Override
    public void visit(SysExpr sysExpr, Object arg) {
        final In1<Node> mapper = sysExpr.voidVisit(this, arg)
            .provide1(this)
            .provide2(arg);

        sysExpr.readAllNodes(mapper, arg);
