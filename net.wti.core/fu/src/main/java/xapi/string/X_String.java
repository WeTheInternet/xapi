...
public class X_String {
  ...
  :js*/ {//java:
    ...
    /// @return A formatted string like "Mon, Jan 15, 2024 @ 9:30am"
    public static String toHumanTimestamp(int year, int month, int date, int hour, int minute) {
        // Calculate day of week using Zeller's congruence formula
        ...
        int dayOfWeek = (date + (13 * (monthZeller + 1)) / 5 + yearZeller + yearZeller / 4 - yearZeller / 100 + yearZeller / 400) % 7;
        ...
        // Build result string with exact size  
        String result = DAYS[dayOfWeek] + ", " + MONTHS[month - 1] + " " + date + ", " + year +
                " @ " + displayHour + ":" + (minute < 10 ? "0" : "") + minute +
                (isPm ? "pm" : "am");
