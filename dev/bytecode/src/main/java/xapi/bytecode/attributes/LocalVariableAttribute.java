...
public class LocalVariableAttribute extends AttributeInfo {
    ...
    public void addEntry(int startPc, int length, int nameIndex,
        ...
        X_Byte.write16bit(nameIndex, newInfo, size + 4);
        X_Byte.write16bit(descriptorIndex, newInfo, size + 6);
        X_Byte.write16bit(index, newInfo, size + 8);
        info = newInfo;
    }
    ...
    void renameClass(String oldname, String newname) {
        ...
        for (int i = 0; i < n; ++i) {
            int pos = i * 10 + 2;
            int index = X_Byte.readU16bit(info, pos + 6);
            if (index != 0) {
                String desc = cp.getUtf8Info(index);
                desc = renameEntry(desc, oldname, newname);
                X_Byte.write16bit(cp.addUtf8Info(desc), info, pos + 6);
            }
        }
    }
    ...
    public int startPc(int i) {
        return X_Byte.readU16bit(info, i * 10 + 2);
    }
    ...
    public int codeLength(int i) {
        return X_Byte.readU16bit(info, i * 10 + 4);
    }
    ...
    public void shiftPc(int where, int gapLength, boolean exclusive) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            int len = X_Byte.readU16bit(info, pos + 2);
    ...
    public int nameIndex(int i) {
        return X_Byte.readU16bit(info, i * 10 + 6);
    }
    ...
    public int descriptorIndex(int i) {
        return X_Byte.readU16bit(info, i * 10 + 8);
    }
    ...
    public int index(int i) {
        return X_Byte.readU16bit(info, i * 10 + 10);
    }
    ...
    public AttributeInfo copy(ConstPool newCp, Map<?, ?> classnames) {
        ...
        int j = 2;
        for (int i = 0; i < n; ++i) {
            int start = X_Byte.readU16bit(src, j);
            int len = X_Byte.readU16bit(src, j + 2);
            int name = X_Byte.readU16bit(src, j + 4);
            int type = X_Byte.readU16bit(src, j + 6);
            int index = X_Byte.readU16bit(src, j + 8);
            ...
            X_Byte.write16bit(type, dest, j + 6);
            X_Byte.write16bit(index, dest, j + 8);
            j += 10;
        }
