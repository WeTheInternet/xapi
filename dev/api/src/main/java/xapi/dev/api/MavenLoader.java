...
public class MavenLoader {
        ...
        default String[] resolveCoordinateNow(String coord, long blocksFor, TimeUnit unit) throws TimeoutException {
            ...
            if (unit == null) {
                ...
                if (System.currentTimeMillis() >= deadline) {
                    throw new TimeoutException("Waited " + blocksFor + " "+ unit + " but MavenLoader did not complete");
                }
                return result.out1();
            }
        }
    ...
    private static final Lazy<MavenLoaderChannel> devJar = Lazy.deferred1Unsafe(()-> {
        ...
        if (home != null) {
            ...
            if (jarFile.exists()) {
                X_Log.info(MavenLoader.class, "Loading xapi-dev from local repo");
                ...
                if (!tmp.exists()) {
                    ...
                    if (!result) {
                        X_Log.warn(MavenLoader.class, "Unable to create tmpDir: ", tmp);
                    }
                }
    ...
    public MvnDependency getDependency(Dependency dependency) {
    ...
    public MvnDependency getDependency(String artifactId) {
        return getDependency(X_Namespace.XAPI_GROUP_ID, artifactId, "jar", null, X_Namespace.XAPI_VERSION);
    }
    public MvnDependency getDependency(String groupId, String artifactId, String version) {
        return getDependency(groupId, artifactId, "jar", null, version);
    }

    public MvnDependency getDependency(String groupId, String artifactId, String type, String classifier, String version) {
