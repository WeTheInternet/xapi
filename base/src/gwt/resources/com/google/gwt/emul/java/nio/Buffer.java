 ...
 * exception. Limit may not be negative and not greater than capacity.</li>
 * <li>Mark: used to remember the current position, so that you can reset the position later. Mark
 * may not be negative and no greater than position.</li>
 * <li>A buffer can be read-only or read-write. Trying to modify the elements of a read-only buffer
 * will cause a <code>ReadOnlyBufferException</code>, while changing the position, limit and mark
 * of a read-only buffer is OK.</li>
 ...
 * callers are responsible to take care of the synchronization issues. </p>
 */
public abstract class Buffer {
    ...
    /** <code>limit - 1</code> is the last element that can be read or written. Limit must be no
     * less than zero and no greater than <code>capacity</code>. */
    int limit;

    /** Mark is where position will be set when <code>reset()</code> is called. Mark is not set by
     * default. Mark is always no less than zero and no greater than <code>position</code>. */
    int mark = UNSET_MARK;

    /** The current position of this buffer. Position is always no less than zero and no greater
    ...
    /** Clears this buffer.
     * <p> While the content of this buffer is not changed, the following internal changes take
     * place: the current position is reset back to the start of the buffer, the value of the
     * buffer limit is made equal to the capacity and mark is cleared. </p>
     *
     * @return this buffer.
     */
    ...
    /** Flips this buffer.
     * <p> The limit is set to the current position, then the position is set to zero, and the mark
     * is cleared. </p>
     * <p> The content of this buffer is not changed. </p>
     *
     * @return this buffer.
     */
     ...
     * <p> If the current position in the buffer is in excess of <code>newLimit</code> then, on
     * returning from this call, it will have been adjusted to be equivalent to
     * <code>newLimit</code>. If the mark is set and is greater than the new limit, then it is
     * cleared. </p>
     *
    ...
    /** Marks the current position, so that the position may return to this point later by calling
     * <code>reset()</code>.
     *
     * @return this buffer.
     */
     ...
     * <p> If the mark is set and it is greater than the new position, then it is cleared. </p>
     *
    ...
    /** Rewinds this buffer.
     * <p> The position is set to zero, and the mark is cleared. The content of this buffer is not
     * changed. </p>
     *
     * @return this buffer.
     */
