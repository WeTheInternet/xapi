 ...
 * <p>
 * Any type that wishes to keep any fields, methods or annotations must have @KeepClass.
 * Without this requirement, the monolithic generator would have to scan every
 * member (field, method, etc) of every type.
 * <p>
...
public @interface KeepClass {
  ...
  /**
   * How to handle Class.newInstance(); default choice is NEW.
   *
   * @return - a {@link NewInstanceStrategy}
   */
  NewInstanceStrategy newInstance() default NewInstanceStrategy.NEW;

  boolean keepPackage() default true;

  boolean keepCodeSource() default false;//security

  boolean keepHierarchy() default false;//code size
