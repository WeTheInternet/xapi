 ...
 */
public final class InternalGradleCache<T extends ExtensionAware> {
    public static final String EXT_NAME = "_xapiInternal";
    private final T from;
    private ExtensionContainer privates;

    public InternalGradleCache(T ext) {
        this.from = ext;
    }

    public InternalGradleCache(T ext, Object foreignCopy) {
        this.from = ext;
        if (foreignCopy != null) {
            try {
    ...
    public <O> boolean contains(
            Class<? super O> publicTypeOptional,
            String key
    ) {
        final ExtensionContainer extensions = from.getExtensions();
        if (publicTypeOptional != null) {
            ...
            key = key + System.identityHashCode(publicTypeOptional);
        }
    ...
    public static <T extends ExtensionAware, O> O buildOnce(Class<? super O> publicType, T source, String key, BiFunction<? super T, String, ? extends O> factory) {
        InternalGradleCache<T> cache = getCache(source);
        return cache.getOrCreate(publicType, source, key, factory);
    }

    @SuppressWarnings("unchecked")
    public static <T extends ExtensionAware> InternalGradleCache<T> getCache(T ext) {
        final ExtensionContainer extensions = ext.getExtensions();
        InternalGradleCache<T> cache;
        final String cacheSuffix = Integer.toString(identityHashCode(InternalGradleCache.class));
        String key = InternalGradleCache.EXT_NAME + cacheSuffix;
        try {
            cache = (InternalGradleCache) extensions.findByName(key);
        } catch (ClassCastException cce) {
            Object o = extensions.findByName(key);
            try {
                cache = new InternalGradleCache<>(ext, o);
            } catch (Exception ignored) {
                // give up. stomp old cache (debugging shows this happens to be buildSrc leaving a cache on root project somehow)
                cache = new InternalGradleCache<>(ext);
            }
        }
