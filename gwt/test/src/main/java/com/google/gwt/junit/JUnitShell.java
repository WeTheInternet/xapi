...
import xapi.dev.util.ClasspathFixer;
...
public class JUnitShell extends DevMode {
  ...
  private static final ClasspathFixer classpathFixer = new ClasspathFixer();
  ...
  public static void runTest(GWTTestCase testCase, TestResult testResult)
    ...
    Thread.currentThread().setContextClassLoader(classpathFixer.get());
  ...
  private static Set<Platform> getBannedPlatforms(Class<?> testClass,
    ...
    if (testClass.getAnnotation(DoNotRunWith.class) != null) {
    ...
    try {
      ...
      if (testMethod.getAnnotation(DoNotRunWith.class) != null) {
