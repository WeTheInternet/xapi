...
public class SingleDimArrayInjector implements MagicMethodGenerator, UnifyAstListener {
  ...
  public JExpression injectMagic(final TreeLogger logger, final JMethodCall methodCall, final JMethod enclosingMethod,
    ...
    findMethods(ast);
    final JClassLiteral clazz = extractClassLiteral(logger, methodCall, 0, ast, false);
    if (clazz == null) {
    ...
    final JNewArray newArr = new JNewArray(info, arrayType, dims, null, classLit);
//    return newArr.makeStatement().getExpr();
  ...
  private void findMethods(final UnifyAstView ast) {
    JDeclaredType arrayType = ast.searchForTypeByBinary(Array.class.getName());
